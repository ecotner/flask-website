...
My professional work involves a tiny bit of descriptive (mostly ad-hoc requests and making plots to show during meetings), but mostly predictive and prescriptive analytics.
...
Even though ATD decided to go with a more complete enterprise solution for dynamic routing, I learned a great deal from the project, and decided to make [my own web application](http://route-optimizer.ml) capable of solving the CVRPTW to demonstrate this knowledge.
The app is hosted on Google Cloud, and 1) runs a docker container via the Cloud Run service to provide an HTTP endpoint on demand to perform the optimization, and 2) hosts a [Valhalla](https://github.com/valhalla/valhalla) server to generate distance/time matrices between delivery stops for more realistic routing.
